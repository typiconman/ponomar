...
class Bible extends JFrame implements DocHandler, ListSelectionListener, ActionListener
{
	...
	public void startElement(String elem, Hashtable table)
	{
		...
		if(elem.equals("INFO"))
		{
			...
			if (curversion.equals(lastversion))
			{
				...
				{
					CurrentFont=new Font((String)table.get("FontFace"),Font.PLAIN,Integer.parseInt(table.get("FontSize").toString()));
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			// save button clicked ...
			helper.SaveHTMLFile(curbook + "_" + curpassage + ".html", "<TITLE>" + curbook + " " + curpassage + "</TITLE>" + text.getText());
		}
		else if (butnum == 6)
		{
	...
	protected void update(String newBook, String newPassage)
	{
		...
		{
			text.setText("<FONT face=\""+DisplayFont+"\" size=\"5\"/>"+"<B>" + (String)books.get(curbook) + " " + formatPassage(curpassage) + "</B><BR>" +Stuff[0]+"</FONT>");
