...
public class Commemoration implements DocHandler
{
	...
	public void readCommemoration(String FileName) //throws IOException
	{
		...
		try
		{
			BufferedReader frf = new BufferedReader(new InputStreamReader(new FileInputStream(helper.langFileFind(Analyse.getDayInfo().get("LS").toString(),FileName+".xml")), "UTF8"));
