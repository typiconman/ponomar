...
public class Main1 extends JFrame implements PropertyChangeListener, DocHandler, HyperlinkListener, ActionListener
{
	...
	private void write()
	{
		...
		*/
		...
		output += "<BR><BR>";
               ...
               /* for(int j=0;j<MenaionReadings.length;j++){
                  OrderedHashtable Reading=(OrderedHashtable) MenaionReadings[j].get("Readings");
                  OrderedHashtable Readings=(OrderedHashtable) Reading.get("Readings");


                for(Enumeration e=Readings.enumerateKeys();e.hasMoreElements();){
                    String element1=e.nextElement().toString();
                    if (element1.equals("VESPERS")){
                        output+="Vespers: ";
                        OrderedHashtable test = (OrderedHashtable) Readings.get("VESPERS");
                        for (Enumeration e1=test.enumerateKeys();e1.hasMoreElements();){
                            String element2=e1.nextElement().toString();
                            OrderedHashtable readV=(OrderedHashtable) test.get(element2);
                            String readingV=readV.get("Reading").toString();

                            output+=ShortForm.getHyperlink(readingV);
