...
public class Matins implements DocHandler {
    ...
    public Matins(OrderedHashtable dayInfo) {
        Information3.setDayInfo(dayInfo);
        ...
        findLanguage=new Helpers(Information3.getDayInfo());
    }
...
//THESE ARE THE SAME FUNCTION AS IN MAIN, BUT TRIMMED FOR THE CURRENT NEEDS
    ...
    public String Readings(OrderedHashtable readingsIn, JDate today) {
       ...
       /* Information3.getDayInfo().put("doy","12");
        ...
        int doy = Integer.parseInt(Information3.getDayInfo().get("doy").toString());
        int dow = Integer.parseInt(Information3.getDayInfo().get("dow").toString());
        int nday = Integer.parseInt(Information3.getDayInfo().get("nday").toString());
        int dRank=Integer.parseInt(Information3.getDayInfo().get("dRank").toString());
    ...
    protected String Display(String a, String b, String c) {
        ...
        if (b.length() > 0) {
            if (output.length() > 0) {
                output += Information3.getDayInfo().get("ReadSep") + " ";
            }
            output += b;
        }
        if (c.length() > 0) {
    ...
    public String format(Vector vectV, Vector vectR, Vector vectT) {
        ...
        Bible ShortForm = new Bible(Information3.getDayInfo());
        try {
            ...
            for (int k = 0; k < vectV.size(); k++) {
                ...
                if (k < vectV.size() - 1) {
                    output += Information3.getDayInfo().get("ReadSep");		//IF THERE ARE MORE READINGS OF THE SAME TYPE APPEND A SEMICOLON!
                }
            }
        } catch (Exception a) {
    ...
    class classifyReadings implements DocHandler {
        ...
        public classifyReadings(OrderedHashtable readingsInA) {
            ...
            ParameterValues.setDayInfo(Information3.getDayInfo());
